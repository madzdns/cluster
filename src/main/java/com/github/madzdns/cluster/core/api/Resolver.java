...
import com.github.madzdns.cluster.core.backend.node.dynamic.INFOResponceErrs;
...
import com.github.madzdns.cluster.core.backend.node.statics.HeartbeatWorker;
import com.github.madzdns.cluster.core.backend.node.statics.impl.HostNode;
import com.github.madzdns.cluster.core.core.NodeNameResolver;
import com.github.madzdns.cluster.core.core.RequestReportServiceCall;
import com.github.madzdns.cluster.core.service.ThreadPoolService;
import com.github.madzdns.cluster.core.utils.Dictionary;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import com.github.madzdns.cluster.core.api.Geo2.Location;
...
import com.github.madzdns.cluster.core.config.GeoMapper;
import com.github.madzdns.cluster.core.config.GeoResolver;
import com.github.madzdns.cluster.core.config.MyEntry;
import com.github.madzdns.cluster.core.config.RequestCount;
...
public class Resolver { 
	...
	updateMappers(
		...
		if(e != null) {
			...
			Node.EntryParam nodeEntryParam = e.getEntryParam(domain);
			...
			if(nodeEntryParam != null) {
				...
				if(isfixed != existingNodeFixedState) {
					...
					log.warn("There was already a Node<core={}, fixed={}> in the dataset that confilicts with new Node<core={}, fixed={}>",
			...
			if(e.getId() == id || id == -1 || e.getId() == -1) {
				...
				//to fix problem when core just got started and nodes are connected
				if(resolverStart > 0) {
				...
				if(!isfixed || (isfixed && theEntry != null)) {
					...
					Node.EntryParam ep = e.updateEntryParams(gm,domain, ndomain, geomaps,
			...
			for(Entry<String, Node.EntryParam> entry:e.getEntryParamsMapper().entrySet())
		...
		Node.EntryParam ep = e.updateEntryParams(gm,domain,ndomain,geomaps,gfactor,
		...
		log.info("putting core {} in node_mapper", server);
	...
	public static ResolveResult getBestServer(final String client_ip,
		...
		if(suggestedServer != null) {
			...
			log.debug("Checking suggested core ip {} for domain {}", suggestedServer, id);
			...
			for(Iterator<INode> it = servingNodes.iterator(); it.hasNext(); ) {
				...
				if(node.getServer().equals(suggestedServer)) {
					...
					if(node.isQuestionedSrvAlive(id, proto, port)) {
						...
						log.debug("Sugested core ip {} was OK for domain {}", suggestedServer, id);
						best_node = node;
					}
		...
		if(best_node == null) {
			...
			if(isUsingProxy) {
				...
				//This way we get nearest core to our core for proxy
		...
		Node.EntryParam ep = best_node.getEntryParam(id);
	...
	private static void resolveNames(final String server,final Node.GeoParams gp,
		...
		if(entry.getFullAName().equals(fullNdomain)) {
			 ...
			 * a loop on core. This prevent it.
			 */
	...
	public static void purgeNodeMapper(final Node node) {
		...
		log.debug("Removing Node[core={}]",node.getServer());
