...
import com.github.madzdns.server.rtmp.RTMPTypes;
...
import org.red5.server.scope.Scope.Builder;
import org.red5.server.service.Call;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import com.github.madzdns.server.rtmpt.RTMPTMinaFilter;
import com.github.madzdns.server.core.api.AsyncLookupResult;
...
import com.github.madzdns.server.core.utils.QueryStringHelper;
import com.github.madzdns.server.core.utils.QueryStringHelper.ParsedQuery;

public class RTMPRedirectAsynchHandler extends RTMPHandler {
	
	private final Logger log = LoggerFactory.getLogger(RTMPRedirectAsynchHandler.class);
					...
					case RECEIVE_AUDIO:
						...
						if(!ready) {
							...
							if(lresult == null) {
								...
								StatusObject result = getStatus(NC_CONNECT_REJECTED);
						...
						if (call instanceof IPendingServiceCall) {
							...
							if(cfg != null) {
								...
								if(apikey != null) {
									...
									AsyncLookupResult alresult = new AsyncLookupResult(host, path, queryString, 3000);
									...
									if(lresult == null) {
										...
										result = getStatus(NC_CONNECT_SUCCESS);
							...
							if(cfg != null) {
								...
								conn.setClient(new FakeClientForConnection(String.valueOf(conn.getId())));
								
								GlobalScope global = new GlobalScope();
								
								global.setName("vod");
								
								Context ctx = new Context(RtmpServer.CTX, "vod");
								
								ctx.setServiceInvoker(new AsyncScopeServiceInvoker(session, statusObjectService));
								
								global.setContext(ctx);
								global.setServer(server);

								server.registerGlobal(global);
								
								IScope s = new Scope(new Builder(global, ScopeType.APPLICATION, "vod", false));
								
								

								boolean c = conn.connect(s);
