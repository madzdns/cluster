...
import server.backend.kafka.consumer.ConsumerHandler;
...
public class KafkaServer implements IServer {
	...
	public void start() throws IOException {
		...
		try {
			log.info("Waiting 2 seconds for zookeeper server");
			Thread.sleep(2000);
		...
		try {
			log.info("Waiting 2 seconds for kafka server");
			Thread.sleep(2000);
		} catch (InterruptedException e) {
		...
		consumerProp.put("value.deserializer", "server.backend.kafka.codec.KafkaByteCoder");
		...
		producerProp.put("value.serializer", "server.backend.kafka.codec.KafkaByteCoder");
